...
import tasmyComponents from './tasmyComponents.js';
import {allowPolishS, allowPolishL,allowPolishZ,allowPolishO, allowPolishE} from '../functions';


const Tasmy =({searchField,click})=>{

	
	
	
	
	const cardArray=tasmyComponents.map((obj,i)=>{
		return(
			<Card key={i} index={i} name={obj.name} click={click} source={tasmyComponents} />
			)
	...
	return(
		<div className='mainComponent'>
			...
			<div className='content_header' style={{width:'100%'}}>
				<h3>TAŚMY SAMOPRZYLEPNE, WYKROJE SAMOPRZYLEPNE</h3>
			</div>
			{adjustCardArray}	
		</div>
		)
